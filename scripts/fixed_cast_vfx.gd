...
func _process(delta):
    ...
    if attack_serial > last_player_attack_serial:
        _spawn_player_cast()
...
func _spawn_player_cast():
    ...
    add_child(trace)
    effects.append({"node": trace, "kind": "trace", "life": 0.85, "max_life": 0.85, "target": target, "damage": 1})
    ...
    while index >= 0:
        ...
        if kind == "cast" or kind == "enemy_cast":
            ...
            if node.global_position.distance_to(destination) < 0.42:
                ...
                game._apply_enemy_hit(target, int(effect["damage"]))
                _remove_effect(index)
                index -= 1
                continue
